...
- **The exit-0 contract now passes *through* cage**, and nothing guarantees cage propagates its child's status. So it isn't trusted: after cage returns, `sesame-session` asks the API again via `sesame-kiosk --check` (one request, no browser). Three verdicts, because "no" has two faces: **0** = unlocked → desktop; **2** = locked but *healthy* (the server answered) → the session script loops back and re-shows the gate — this is the kid who unlocked one second before curfew and lost the race to cage teardown; exiting 1 there used to kill SDDM's autologin and leave a black screen. **1** = broken (silent server, missing cage, crashed kiosk) → exit 1, an adult must come. A rapid-death counter (5 gate deaths under 15 s each) demotes a crash-looping cage from verdict 2 to refusal. **A lock that fails open is not a lock — and a black screen is never a valid state.** Never let the desktop start on anything but a positive answer from `policy::evaluate`.
- **The blocked page has a parent corner** (`/parent-unlock`): a faint 🔑 opens a form — admin password only (argon2-verified, 2 s pause on failure). Success flips `AppState::parent_mode`, an **in-memory** `AtomicBool`: `/api/gate` then answers "unlocked, no child", so the kiosk opens the desktop, the timer never debits or cuts (no `child_id` → its idle branch), and **no child's ledger is touched** — the adult isn't borrowing anyone's time. It is deliberately NOT a grant and NOT persisted: the server lives and dies with the session, so logging out or rebooting re-arms the gate *by construction*; storing it in the DB would let a forgetful parent leave the machine open forever. The form says all this out loud. During curfew the desktop doesn't exist and /admin isn't reachable from the gate; without this, the VT console was the parent's only way in.
